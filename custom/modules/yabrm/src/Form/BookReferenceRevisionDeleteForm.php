...
/**
 * Provides a form for deleting a Book reference revision.
 *
 * @ingroup yabrm
 */
class BookReferenceRevisionDeleteForm extends ConfirmFormBase {
  ...
  /**
   * Constructs a new BookReferenceRevisionDeleteForm.
   *
  ...
  public static function create(ContainerInterface $container) {
    ...
    return new static(
      $entity_manager->getStorage('yabrm_book'),
  ...
  public function getFormId() {
    return 'yabrm_book_revision_delete_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    \Drupal::messenger()->addMessage(t('Revision from %revision-date of Book reference %title has been deleted.', ['%revision-date' => format_date($this->revision->getRevisionCreationTime()), '%title' => $this->revision->label()]));
    $form_state->setRedirect(
      'entity.yabrm_book.canonical',
       ['yabrm_book' => $this->revision->id()]
    );
    if ($this->connection->query('SELECT COUNT(DISTINCT vid) FROM {yabrm_book_field_revision} WHERE id = :id', [':id' => $this->revision->id()])->fetchField() > 1) {
      $form_state->setRedirect(
