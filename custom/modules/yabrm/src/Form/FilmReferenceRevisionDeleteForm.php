...
/**
 * Provides a form for deleting a Film reference revision.
 *
 * @ingroup yabrm
 */
class FilmReferenceRevisionDeleteForm extends ConfirmFormBase {
  ...
  /**
   * Constructs a new FilmReferenceRevisionDeleteForm.
   *
  ...
  public static function create(ContainerInterface $container) {
    ...
    return new static(
      $entity_manager->getStorage('yabrm_film'),
  ...
  public function getFormId() {
    return 'yabrm_film_revision_delete_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    $this->filmReferenceStorage->deleteRevision($this->revision->getRevisionId());

    $this->logger('content')->notice('Film reference: deleted %title revision %revision.', [
      '%title' => $this->revision->label(),
      '%revision' => $this->revision->getRevisionId(),
    ]);
    $this->messenger()->addMessage($this->t('Revision from %revision-date of Film reference %title has been deleted.', [
      '%revision-date' => $this->dateFormatter->format($this->revision->getRevisionCreationTime()),
      '%title' => $this->revision->label(),
    ]));
    $form_state->setRedirect(
      'entity.yabrm_film.canonical',
       ['yabrm_film' => $this->revision->id()]
    );
    if ($this->connection->query('SELECT COUNT(DISTINCT vid) FROM {yabrm_film_revision} WHERE id = :id', [':id' => $this->revision->id()])->fetchField() > 1) {
      $form_state->setRedirect(
